...
let help () =
  ...
  printf "HMM model:\n" ;
  printf
    "-t,  --tag-order=NUM          Order of tag transitions. NUM=2 means \
     trigram tagging. (default: -t2)\n" ;
  printf
    "-e,  --emission-order=NUM     NUM=1 -> P(w_i | t_i), NUM=2 ->P(w_i | \
     t_{i-1} t_i). (default: -e2)\n" ;
  printf "\n" ;
  printf "Unknown words:\n" ;
  printf
    "-s,  --suffix-length=LEN      use suffix trie with max. suffix length = \
     LEN; (default = -s10)\n" ;
  printf
    "-f,  --rare-frequency=NUM     add only words to the suffix trie with \
     max. freq < NUM; (default = -f10)\n" ;
  printf "\n" ;
