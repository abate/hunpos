...
let usage () = Printf.eprintf "usage : %S config_file \n" Sys.argv.(0)
...
let _ =
  if Array.length Sys.argv < 2 then
    ...
    let tagger =
      Hmm_tagger.compile_tagger model (Morphtable.tags hunmorph) tagorder
        emorder max_guessed_tags
    in
    ...
    for i = 0 to 1 do
      for j = 0 to 1 do
        prec.(i).(j)
        <- float (total_matrix.(i).(j) - false_matrix.(i).(j))
           *. 100.0
