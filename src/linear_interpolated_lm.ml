...
module Make (M : Maps) :
  S with type word = M.WMap.key and type context = M.CMap.key = struct
  type word = M.WMap.key

  type context = M.CMap.key

  type t =
    | Parent of (float * t M.CMap.t * float M.WMap.t)
...
*)
  let iterate_paths f context_node max_level =
    let rec aux node acc level =
      if level = max_level then f level (node :: acc)
      else
        (* meg megyunk lefele *)
        match node with
  ...
  let _iter_ngrams f trie =
    let rec aux node acc =
      ...
      match node with
      | Parent (_, childs, _) ->
          let do_child gram node = aux node (gram :: acc) in
    ...
    let adjust_lambda level context_nodes =
      ...
      let do_word word freq =
        let rec search_max nodes i max maxi =
          match nodes with
          | node :: tail ->
              let context_freq, words =
                match node with
                | Terminal (freq, words) -> (freq, words)
                | Parent (freq, _, words) -> (freq, words)
              in
              let word_freq =
                try M.WMap.find words word with Not_found ->
                  failwith "error in the tree"
              in
              let ratio =
              ...
              in
              let max, maxi =
                if ratio > max then (ratio, i) else (max, maxi)
              in
