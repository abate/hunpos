...
let proc_err s = prerr_string s ; exit 1
...
let help () =
  ...
  printf
    "-m,  --morphtable=FILE        Morphological lexicon. (default: do not use)\n" ;
  printf "\n" ;
  printf "Performance tuning:\n" ;
  printf
    "-g,  --max-guessed-tags=NUM   only the most probable NUM tags used \
     (default = -g10)\n" ;
  printf
    "-b,  --beam-theta=NUM              use NUM as theta in viterbi beam \
     search (default = -t1000)\n" ;
  printf "\n" ;
  printf "Output options:\n" ;
  printf
    "-t,  --tokens-types           print the types of each token (default = no)\n" ;
  printf
...
let tag_sentence marktokens tagger sentence =
  ...
  let print_tagged obs tag =
    print_string obs.Hmm_tagger.word ;
    print_char '\t' ;
    print_string tag ;
    print_char '\t' ;
    if marktokens then print_endline (typ2string obs.Hmm_tagger.seen)
    else print_newline ()
  in
...
let main () =
  ...
  let tagger =
    Hmm_tagger.compile_tagger model hunmorph !max_guessed_tags
      (log (float_of_int !theta))
  in
  if !verbose then prerr_endline "tagger compiled" ;
  let ic = stdin in
  Io.iter_sentence ic (tag_sentence !marktokens tagger)

let _ = main () ; Gc.full_major ()
