...
let extract_arg_handle opt = function
  ...
  | _ ->
      raise (Error (Printf.sprintf "Option %s does not accept argument" opt))

let extract_handle opt = function
  | _, _, Some handle, _ -> handle
  | _ -> raise (Error (Printf.sprintf "Option %s must have an argument" opt))

let parse opts others args first last =
  let find_long opt =
    try List.find (fun (_, l, _, _) -> opt = l) opts with Not_found ->
      raise (Error (Printf.sprintf "Unknown option --%s" opt))
  in
  let find_short opt =
    try List.find (fun (l, _, _, _) -> opt = l) opts with Not_found ->
