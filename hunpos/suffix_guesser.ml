...
type trie_node =
  | Terminal
  | OneChild of char * count_node
  | Branch of count_node C.t

and counts = int * int T.t

and count_node = Empty | Node of trie_node * counts option
...
let calculate_theta apriori_tag_probs =
  ...
  (* we follow libmmoot's solution here, not Brants 2000's formula.
	...
	*)
  let p_av =
    Array.fold_left (fun sum tagp -> sum +. pow tagp) 0.0 apriori_tag_probs
  in
  let theta =
    Array.fold_left
      (fun sum tagp -> sum +. (tagp *. pow (tagp -. p_av)))
      0.0 apriori_tag_probs
  in
  sqrt theta
...
let guesser_from_trie trie theta =
  ...
  let trie_iterator word f =
    ...
    let rec aux t legacy_counts ix =
      ...
      if ix >= stop then
        ...
        | Terminal -> ()
        | OneChild (c, child) ->
            if c = word.[ix] then aux child inherited_counts (ix - 1)
        | Branch childs -> (
          try
  ...
  let tag_prob word tagid =
    ...
    let roll_prob suff_count tag_counts =
      let tag_prob =
        try float (T.find tagid tag_counts) /. suff_count with Not_found -> 0.0
      in
