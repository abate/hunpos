...
let start_train tag_order emission_order =
  ...
  let m =
    ...
    ; theta= 0.0 }
  in
  let stat = {tokens= 0; sentences= 0; types= 0; rare_low= 0; rare_upp= 0} in
  (m, stat)
...
let calculate_probs (m, stat) =
  ...
  TagProbLM.iter_words
    (fun tag freq ->
      try m.apriori_tag_probs.(tag) <- freq /. total_freq with _ ->
        Printf.printf "error %d %s\n" tag (Vocab.toword m.tag_vocab tag) )
    m.tag_lm ;
  ...
  let do_word word freq tags =
    if freq <= maxfreq then
      ...
      let add_tag (tag, freq) =
        if is_upper then (
          m.upp_suffixes
          <- Suffix_guesser.add_word m.upp_suffixes maxlength lword tag freq ;
          stat.rare_upp <- stat.rare_upp + freq )
        else (
          m.low_suffixes
          <- Suffix_guesser.add_word m.low_suffixes maxlength lword tag freq ;
...
let print_stat (m, stat) =
  ...
  prerr_int stat.tokens ;
  prerr_endline " tokens" ;
  prerr_int stat.sentences ;
  prerr_endline " sentences" ;
  prerr_int (Vocab.max m.tag_vocab) ;
  prerr_endline " different tag" ;
  prerr_endline "\nGuesser trained with" ;
  prerr_int stat.rare_low ;
  prerr_endline " lowercase " ;
  prerr_int stat.rare_upp ;
  prerr_endline " uppercase tokens" ;
  prerr_string "theta = " ;
  prerr_float m.theta ;
  prerr_newline ()
...
type observation =
  ...
  ; (* eleg ronda hack arra, h az elso szot kisbetusithessuk *)
...
let compile_tagger (m, stat) morphtable max_guessed_tags logtheta =
  ...
  let module StateViterbi = Viterbi.Make (Mfhash.Make (State)) in
  let start_state =
    let rec aux n l = if n < 0 then l else aux (n - 1) (m.bos :: l) in
    aux tag_order []
  in
  let end_of_sentence =
    let transition from =
  ...
  let next obs =
    ...
    if w = "<s>" then end_of_sentence
      ...
      (* is it known words? *)
      let oov, anals =
        try (false, morphtable w) with Not_found -> (true, [])
      in
      obs.oov <- oov ;
      obs.anals <- anals ;
      ...
      with Not_found ->
        ...
        if List.length obs.anals = 1 then
          ...
          let tagprobs, tagprob =
            if is_upper then (utagprobs, utagprob) else (ltagprobs, ltagprob)
          in
          if not obs.oov then
            let transition = anals2transtion_fun obs.anals in
            let emission state =
              ...
              else tagprob lw tag -. log m.apriori_tag_probs.(tag)
            in
            (transition, emission)
          else
                ...
                (fun (tagid, w) ->
                  ...
                  if debug then
                    Printf.printf "trans %s %f eredeti w=%f\n"
                      (Vocab.toword m.tag_vocab tagid)
                      tp w ;
                  (next_state, tp) )
                pruned_guessing
            in
  ...
  let tag_sentence words =
    ...
    let state_seq =
      StateViterbi.decode next logtheta start_state observations
    in
    let state_seq =
      List.map
        (fun state ->
          let tag_id = Ngram.newest state in
          Vocab.toword m.tag_vocab tag_id )
        state_seq
    in
