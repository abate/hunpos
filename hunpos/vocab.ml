...
let toword (_word2id, id2word, _max) idx = IHash.find id2word idx

let ngram_toindex vocab ngram = List.map (toindex vocab) ngram

let max (_, _, max) = !max

(*
let fix_old_vocab (_, old_id2word, _max) =
