...
struct
  ...
  type 'b bucketnode =
    {mutable next: 'b bucketlist; key: key; mutable value: 'b}

  and 'b bucketlist = Empty | Cons of 'b bucketnode

  type 'b t =
    { mutable size: int
    ; (* size *)
      mutable hash_mask: int
    ; (* hany bites a hash fgv. *)
...
*)
              ...
              | Cons node ->
                  ...
                  let rec aux prev =
                    ...
                    | Empty ->
                        prev.next
                        <- Cons
                             {next= Empty; key= onode.key; value= onode.value}
  ...
  let print_bucket_stat h =
    ...
    for i = 0 to Array.length d - 1 do
      ...
      try incr (Hashtbl.find hist len) with Not_found ->
        Hashtbl.add hist len (ref 1)
    done ;
  ...
  let update init updater h k =
    ...
    match l with
    | Empty ->
        (* the bucket is empty just add the new element *)
        let nv = init () in
          ...
          let rec update_bucket_list prev =
            ...
            | Cons cur ->
                if H.equal k cur.key then (
                  ...
                  cur.value <- nv ;
                  (* move front the node *)
                  if h.move_to_front then (
                    prev.next <- cur.next ;
                    cur.next <- Cons first ;
                    h.data.(i) <- Cons cur ) ;
                  nv )
                else update_bucket_list cur
          in
...
*)
  ...
  let find h k = update (fun () -> raise Not_found) (fun x -> x) h k

  let add_or_replace h k v =
    let _ = update (fun () -> v) (fun _ -> v) h k in
    ()

  let find_or_add h k v = update (fun () -> v) (fun x -> x) h k
end
